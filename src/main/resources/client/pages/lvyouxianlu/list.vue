...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","width":"100%","padding":"20rpx 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
			<view :style='{"padding":"20rpx 0 0 0","alignItems":"center","flex":"1","display":"flex","width":"calc(98% - 120rpx)","position":"relative","justifyContent":"space-between"}'>
				<view :style='{"backgroundColor":"#f5f5f5","margin":"0 30rpx 0 30rpx","color":"#333333","alignItems":"center","borderRadius":"0px","flex":"1","display":"flex","lineHeight":"64rpx","fontSize":"24rpx","height":"64rpx"}'>
					...
					<input v-model="searchForm.xianlumingcheng" type="text" placeholder="线路名称" :style='{"background":"transparent","height":"100%"}'></input>
				</view>
			...
			<view class="uni-product-list" :style='{"padding":"24rpx 24rpx 24rpx 24rpx","margin":"40rpx 0 0 0","alignItems":"flex-start","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"0 0px 0px #eeeeee","margin":"0 0 40rpx 0","backgroundColor":"#ffffff","borderRadius":"8rpx","flexWrap":"wrap","textAlign":"center","display":"flex","width":"48%","justifyContent":"center","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view class="uni-product-title" :style='{"padding":"0px 20rpx 0px 20rpx","margin":"0 0 0px 0","whiteSpace":"nowrap","color":"#333333","textAlign":"left","overflow":"hidden","borderRadius":"8rpx","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","textOverflow":"ellipsis","order":"2","height":"72rpx"}'>线路名称:{{product.xianlumingcheng}}</view>
					...
					<view :style='{"padding":"0","margin":"8rpx 0 0 0","display":"flex","width":"100%","justifyContent":"space-between","height":"auto","order":"3"}'>
						<view :style='{"border":"0px solid #dcf8f5","padding":"0px 20rpx 20rpx 20rpx","borderRadius":"60rpx","display":"flex"}' v-if="(userid && isAuth('lvyouxianlu','修改')) || (!userid && isAuthFront('lvyouxianlu','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"border":"0px solid #f9dede","padding":"0px 20rpx 20rpx 20rpx","borderRadius":"60rpx","display":"flex"}' v-if="(userid && isAuth('lvyouxianlu','删除')) || (!userid && isAuthFront('lvyouxianlu','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 0px 0px #cccccc","color":"#ffffff","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#3da742","width":"80rpx","lineHeight":"80rpx","fontSize":"26rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('lvyouxianlu','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 0px 0px #cccccc","color":"#ffffff","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#3da742","width":"80rpx","lineHeight":"80rpx","fontSize":"26rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('lvyouxianlu','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
	...
	<view :style='{"width":"80%","padding":"40rpx 20rpx","background":"#fdfbd9"}' class="screenBox" :class="screenBoxShow?'screenBoxActive':''">
		<view :style='{"width":"100%","padding":"20rpx 20rpx 20rpx 0","alignItems":"center","justifyContent":"space-around","display":"flex"}'>
			<view :style='{"width":"18%","padding":"0 0 0 20rpx","fontSize":"24rpx"}'>目的地</view>
			<input :style='{"border":"2rpx solid #dddddd","padding":"0 20rpx 0 20rpx","borderRadius":"8rpx","color":"#666666","background":"#ffffff","width":"80%","height":"60rpx"}' placeholder="请输入目的地" v-model="searchForm.mudedi">
		</view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.xianlumingcheng="";
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.xianlumingcheng){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`lvyouxianlu`, params);
                } else {
                    res = await this.$api.list(`lvyouxianlu`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('lvyouxianlu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.xianlumingcheng){
					searchForm['xianlumingcheng'] = '%' + this.searchForm.xianlumingcheng + '%'
				}
				if(this.searchForm.mudedi){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`lvyouxianlu`, searchForm);
                } else {
                    res = await this.$api.list(`lvyouxianlu`, searchForm);
                }
				// 如果是第一页数据置空
